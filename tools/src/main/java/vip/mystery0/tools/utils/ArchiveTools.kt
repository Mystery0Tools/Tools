...
class ArchiveTools private constructor(private val type: Int) {
	...
	suspend fun compress() {
		when (type) {
			TYPE_TAR_GZ -> compressDir.tarGz(archiveFileName, savePath, suffix, isDeleteExistFile)
			TYPE_ZIP -> compressDir.zip(archiveFileName, savePath, suffix, isDeleteExistFile)
	...
	suspend fun decompress() {
		when (type) {
			TYPE_TAR_GZ -> archiveFile.unTarGz(decompressDir, isDelete)
			TYPE_ZIP -> archiveFile.unZip(decompressDir, isDelete)
			else -> throw NoSuchElementException("类型错误")
		}
	}
